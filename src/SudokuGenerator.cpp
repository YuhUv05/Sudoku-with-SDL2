...
void Sudoku::Generator::createCompletedSudoku()
{
	...
	// Fill first row with numbers 1 to 9
	for (int i = 0; i < 9; i++)
	{
		mGrid[i] = i + 1;
	}

	// Shuffle first row of 9 numbers
	int swaps = 50;
	...
	// Fill second and third row by previous row but shifted by 3
	fillNextRow(0, 1, 3);
	fillNextRow(1, 2, 3);

	// Fill fourth row by previous row but shifted by 1
	fillNextRow(2, 3, 1);

	// Fill fith and sixth row by previous row but shifted by 3
	fillNextRow(3, 4, 3);
	fillNextRow(4, 5, 3);

	// Fill seventh row by previous row but shifted by 1
	fillNextRow(5, 6, 1);

	// Fill eith and ninth row by previous row but shifted by 3
	fillNextRow(6, 7, 3);
	fillNextRow(7, 8, 3);

	// Shuffle rows within every row block
	int shuffles = 10;
	...
	// Shuffle cols within every col block
	...
	// Store solution in solution grid
...
void Sudoku::Generator::generate(int* grid, int* solutionGrid)
{
	...
	// Elements to remove (between 30 and 40, difficulty randomized)
