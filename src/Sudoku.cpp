...
Sudoku::Sudoku::Sudoku()
	: mWindowHeight(800), mWindowWidth(720),
	  mGridHeight(630), mGridWidth(720),
...
bool Sudoku::Sudoku::initialiseSDL()
{
	...
	// Load font for text
	mFont = TTF_OpenFont("assets/OpenSans-Bold.ttf", mFontSize);
	if (mFont == nullptr)
	{
...
void Sudoku::Sudoku::createInterfaceLayout()
{
	...
	// mGridWidth = 6 * thinBorder + 4 * thickBorder + 9 * buttonWidth
	...
	for (int gridRow = 0; gridRow < mGridRows; gridRow++)
	{
		...
		if (gridRow == 0) 
			...
			(gridRow % 3 == 0) buttonStartRow += buttonHeight + thickBorder;
		else
			buttonStartRow += buttonHeight + thinBorder;
		...
		for (int gridCol = 0; gridCol < mGridCols; gridCol++)
		{
			...
			if (gridCol == 0) 
				...
				(gridCol % 3 == 0) buttonStartCol += buttonWidth + thickBorder;
			else 
				buttonStartCol += buttonWidth + thinBorder;
	...
	// mGridWidth = 3 * thickBorder + 9 * numberOfOtherButtons
...
{
	// Create an empty grid to store generated Sudoku
	int generatedGrid[81] = { };

	// Create an empty grid to store solution to generated Sudoku
	int solution[81] = { };
